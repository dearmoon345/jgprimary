	...
	public function add(){
        ...
        try{
        	...
        	if($userId==0){
		        ...
		        if($tpl['tplContent']!=''){
		            $params = ['tpl'=>$tpl,'params'=>['MALL_NAME'=>WSTConf("CONF.mallName"),'LOGIN_NAME'=>$user['loginName'],'LOGIN_PASS'=>$loginPwd]];
		            $rv = model('admin/LogSms')->sendSMS(0,$userId,$userPhone,$params,'handle');
		        }
        	}else{
        		//发送短信消息
		        $tpl = WSTMsgTemplates('PHONE_SHOP_OPEN_SUCCESS');
		        if($tpl['tplContent']!=''){
		            $params = ['tpl'=>$tpl,'params'=>['MALL_NAME'=>WSTConf("CONF.mallName")]];
		            $rv = model('admin/LogSms')->sendSMS(0,$userId,$userPhone,$params,'handle');
		        }
				...
				if((int)WSTConf('CONF.wxenabled')==1){
					$userInfo = model('ShopApplys')->field('createTime')->find();
					$params = [];
