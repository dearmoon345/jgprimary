...
class Orders extends Base{
	...
	public function toExport(){
		...
		->field('o.orderId,o.orderNo,u.loginName,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,o.deliverMoney,lo.logContent
	...
	*/
	public function toExcel(){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', '经销商ID')->setCellValue('B1', '姓名')->setCellValue('C1', '省份')->setCellValue('D1', '本月佣金');
		$objPHPExcel->getActiveSheet()->getStyle('A1:P1')->applyFromArray($styleArray);
	
		for ($row = 0; $row < count($info); $row++){
			$i = $row+2;
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, $info[$row]['userId'])->setCellValue('B'.$i, $info[$row]['userName'])->setCellValue('C'.$i, $info[$row]['Province'])->setCellValue('D'.$i, $info[$row]['CommissionFee']);
		}
		...
		header("Content-Type:application/vnd.ms-excel;");
		header("Content-Type:application/octet-stream");
		header('Content-Disposition: attachment;filename="经销商列表.xls"');
		header("Content-Transfer-Encoding:binary");
		$objWriter->save('php://output');
		}
	/**
	...
	*/
	public function excelProvince(){
		...
		header('Content-Disposition: attachment;filename="省经销商列表.xls"');
		header("Content-Transfer-Encoding:binary");
		$objWriter->save('php://output');
	}
}
