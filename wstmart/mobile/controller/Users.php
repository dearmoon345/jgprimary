...
class Users extends Base{
    ...
    protected $beforeActionList = [
          'checkAuth' =>  ['except'=>'checklogin,login,register,getverify,toregister,forgetpass,forgetpasst,forgetpasss,forgetpassf,findpass,getfindphone,resetpass,getphoneverifycode']// 访问这些except下的方法不需要执行前置操作
    ];
    /**
     * 会员登录页
     */
    public function login(){
        ...
        if(!empty($USER) && $USER['userId']!=''){
            $this -> assign('clientType', session('clientType'));
    ...
    */
    ...
    public function ceshiUser(){
        ...
        $find = Db::name('cheshi_user')->where('userPhone',$info['userPhone'])->find();  
        if ($find) {
            return WSTReturn('success',1);
        }
