...
class GoodsCats extends Base{
	...
	public function getGoodsCats(){
		...
		if(!$list){
			//查询一级分类
			$trs1s = $this->where(["dataFlag"=>1,"isShow"=>1,"isFloor"=>1,"parentId"=>0])->field('catId,parentId,catName')->order('catSort asc')->select();
			$trs1 = array();
			$list = array();
			$rs2 = array();
			$maprs = array();
			...
			//查询二级分类
			$trs2s = $this->where("dataFlag=1 and isShow=1 and isFloor=1 and parentId in(".implode(',',$ids).")")->field('catId,parentId,catName')->order('catSort asc')->select();
			$trs2 = array();
			$ids = array();
			...
			//查询三级分类
			$trs3s = $this->where("dataFlag=1 and isShow=1 and isFloor=1 and parentId in(".implode(',',$ids).")")->field('catId,parentId,catName')->order('catSort asc')->select();
			$trs3 = array();
			$ids = array();
